    ...
    return (
      <div style={{ position: "relative", width: "80px", height: "96px", paddingBottom: "16px", isolation: "isolate" }}>
        ...
        <div style={{
          ...
          {isVision ? (
            <img src={getImageUrl(imgSrc)} alt="" style={{ width: `${imgW}px`, height: `${imgH}px`, objectFit: "cover" }} />
          ) : (
            <div style={{
              ...
              <img src={getImageUrl(imgSrc)} alt="" style={{ width: `${imgW}px`, height: `${imgH}px`, objectFit: "contain" }} />
            </div>
          )}
        </div>
      </div>
  ...
  if (loading) {
    return (
      <section className="about-missionvision" style={{
        width: "100%", padding: "59px 113.5px 67px", background: "#FFFFFF",
        fontFamily: "'Inter', sans-serif", position: "relative", minHeight: "560px"
      }}>
        {/* Skeleton/Loading layout */}
        <div style={{ textAlign: "center", marginTop: "100px", color: "#666" }}>Loading...</div>
      </section>
    );
  }
    ...
    <section className="about-missionvision" style={{
      ...
      {(data?.badgeText || data?.sectionTitle) && (
        <div style={{ textAlign: 'center', marginBottom: '40px' }}>
          {data?.badgeText && <span style={{ color: "#FFB952", fontWeight: 600, display: "block", marginBottom: "8px" }}>{data.badgeText}</span>}
          {data?.sectionTitle && <h2 style={{ fontSize: "36px", fontWeight: 800, color: "#000000", margin: 0 }}>{data.sectionTitle}</h2>}
          {data?.description && <p style={{ color: "#666", marginTop: "16px", maxWidth: "800px", margin: "16px auto 0" }}>{data.description}</p>}
        </div>
      )}

      <div className="about-mv-container" style={{ width: "1213px", maxWidth: "100%", height: "438.25px", position: "relative", margin: "0 auto" }}>
        {/* Mission Card */}
        <div className="about-mv-mission" style={{
          ...
          <div style={{
            ...
            {mission.cta && (
              <a href={mission.cta.link || '#'} style={{ color: "#005CB9", fontWeight: 600, textDecoration: "none", marginTop: "16px" }}>
                {mission.cta.text}
              </a>
            )}
          </div>
        ...
        <div className="about-mv-vision" style={{
          ...
          <div style={{
            ...
            {vision.cta && (
              <a href={vision.cta.link || '#'} style={{ color: "#FFB952", fontWeight: 600, textDecoration: "none", marginTop: "16px" }}>
                {vision.cta.text}
              </a>
            )}
          </div>
      ...
      {data?.ctaButton && (
        <div style={{ display: "flex", justifyContent: "center", marginTop: "40px" }}>
          <a href={data.ctaButton.link || '#'} style={{ padding: "12px 24px", background: "#005CB9", color: "#fff", borderRadius: "8px", textDecoration: "none", fontWeight: 600 }}>
            {data.ctaButton.text}
          </a>
        </div>
      )}
